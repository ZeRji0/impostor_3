...
let coopUsed = new Set();           // palabras ya usadas en rondas previas
...
function setupRoleScreenForPlayer(index) {
  ...
  if (impostors.includes(index)) {
    ...
    roleTextElem.textContent = secretWord;
  }
  roleTextElem.style.display = 'none';
  showRoleBtn.disabled = false;
  hideRoleBtn.disabled = true;
...
function computeCoopPoolForThisRound() {
  // Palabras aportadas por quienes NO son impostores en esta ronda
  ...
  if (!pool.length) {
    // Fallback: cualquier aportación no usada de cualquiera
    let all = [];
...
function assignRolesAndPickWord() {
  ...
  if (wordMode === 'random') {
    // Asíncrono fuera: el que llama gestionará la promesa
    return 'random';
  }
...
function pasteConfiguration() {
  ...
  try {
    ...
    if (impostorMode === 'fixed') {
      impFixedRadio.checked = true; impRandomRadio.checked = false; impCountInput.disabled = false; impCountInput.value = impostorCount;
    } else {
      impFixedRadio.checked = false; impRandomRadio.checked = true; impCountInput.disabled = true;
    }
...
startButton.addEventListener('click', () => {
  ...
  if (wordMode === 'coop') {
    // Preparar o reutilizar estructura de entradas (si no hay contribuciones previas)
    ...
    players.forEach(player => {
      ...
      // Si ya existían contribuciones, precárgalas
      if (coopContributionsByPlayer[player]) {
        textarea.value = coopContributionsByPlayer[player].join('\n');
      }
...
coopContinueButton.addEventListener('click', () => {
  // Guardar/actualizar contribuciones
  coopContributionsByPlayer = {};
  players.forEach(p => {
    const t = document.getElementById("words_" + p);
    const words = t ? t.value.split('\n').map(w => w.trim()).filter(Boolean) : [];
    coopContributionsByPlayer[p] = words;
  });
  // Primera ronda cooperativa
  const r = assignRolesAndPickWord();
  if (r) startRoleScreen();
  else if (r === 'random') { getRandomSpanishWord().then(w => { secretWord = w; startRoleScreen(); }); }
});

// Controles de cada jugador (pantalla de roles)
...
finishRoundButton.addEventListener('click', () => {
  if (competitive) {
    ...
    // Nueva ronda manteniendo configuración y (si coop) las mismas palabras sin repetir
    if (wordMode === 'coop') {
      const r = assignRolesAndPickWord();
      if (r) startRoleScreen();
      else if (r === 'random') { getRandomSpanishWord().then(w => { secretWord = w; startRoleScreen(); }); }
...
newRoundButton.addEventListener('click', () => {
  // Nueva ronda desde el menú principal
  if (wordMode === 'coop') {
    // Seguir usando las mismas palabras, sin repetir y respetando regla del impostor
    const r = assignRolesAndPickWord();
    if (r) startRoleScreen();
    else if (r === 'random') { getRandomSpanishWord().then(w => { secretWord = w; startRoleScreen(); }); }
  } else {
